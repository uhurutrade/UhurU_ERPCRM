...
async function main() {
    console.log('🤖 Starting System Data Vectorization...');
...
${settings.notes || 'None'}
        ...
        // 4. Recent Transactions (Last 50 globally)
        ...
        // 5. CRM Organizations & Contacts (Basic Overview)
        const organizations = await prisma.organization.findMany({ take: 20 });
        let crmContent = "KEY CRM ORGANIZATIONS\n---------------------\n";
        for (const org of organizations) {
            crmContent += `Name: ${org.name}\nSector: ${org.sector || 'N/A'}\nEmail: ${org.email || 'N/A'}\nBillable: ${org.isBillable ? 'Yes' : 'No'}\n\n`;
        }
        await ingestText('sys_crm_organizations', 'CRM Organizations Overview', crmContent);

        console.log('✅ System Data Vectorization Complete.');
